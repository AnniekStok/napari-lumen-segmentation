...
class SaveLabelsWidget(QWidget):
    ...
    def __init__(
        ...
        # Split slices
        self.split_time_points = QCheckBox("Split frames")
        self.split_time_points.setEnabled(
            isinstance(self.layer_manager.selected_layer, napari.layers.Labels)
            and self.layer_manager.selected_layer.data.ndim >= 3
        )
        self.layer_manager.layer_update.connect(
        ...
        self.layer_manager.layer_update.connect(self.update_filename)
        ...
        self.save_btn.setEnabled(isinstance(self.layer_manager.selected_layer, Labels))
        self.layer_manager.layer_update.connect(
    ...
    def update_filename(self) -> None:
        ...
        if isinstance(self.layer_manager.selected_layer, napari.layers.Labels):
            self.filename.setText(self.layer_manager.selected_layer.name)

    def _save_labels(self) -> None:
        """Save the currently active labels layer. If it consists of multiple timepoints, they are written to multiple 3D stacks."""

        data = self.layer_manager.selected_layer.data
        ndim = data.ndim
