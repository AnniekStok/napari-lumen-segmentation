...
from qtpy.QtWidgets                         import QScrollArea, QGroupBox, QMessageBox, QLabel, QHBoxLayout, QVBoxLayout, QPushButton
...
class DistanceWidget(QScrollArea):
    ...
    def __init__(self, viewer:napari.Viewer, labels: Labels):
        ...
        self.geodesic_distmap_mask_dropdown.layer_changed.connect(self._update_geodesic_distmap_mask)
        ...
        self.geodesic_distmap_marker_layer_dropdown = LayerDropdown(self.viewer, (Labels, Points))
        self.geodesic_distmap_marker_layer_dropdown.layer_changed.connect(self._update_geodesic_distmap_marker_layer)
        geodesic_distmap_marker_layer_layout.addWidget(self.geodesic_distmap_marker_layer_dropdown)
    ...
    def _calculate_geodesic_distance(self): 
        ...
        if type(self.geodesic_distmap_mask_layer) == da.core.Array == da.core.Array:
                msg = QMessageBox()
        ...
        if isinstance(self.geodesic_distmap_marker_layer, Labels):
            ...
            self.viewer.add_image(np.array(dip.GeodesicDistanceTransform(~marker, self.geodesic_distmap_mask_layer.data > 0), dtype = np.float32), colormap = 'magma')
            ...
            if len(self.geodesic_distmap_marker_layer.data) == 1: 
                ...
                mask2[tuple(self.geodesic_distmap_marker_layer.data[0].astype(int))] = False
                self.viewer.add_image(np.array(dip.GeodesicDistanceTransform(mask2, self.geodesic_distmap_mask_layer.data > 0), dtype = np.float32), colormap = 'magma')
                ...
                for point1, point2 in combinations(self.geodesic_distmap_marker_layer.data, 2):
                    ...
                    dist_map = np.array(dip.GeodesicDistanceTransform(mask2, self.geodesic_distmap_mask_layer.data > 0), dtype = np.float32)
                    ...
                    measurements = pd.concat([measurements, pd.DataFrame([measurement_dict])])
                ...
                for index, row in measurements.iterrows():
                    ...
                    for j in point1_cols:
                        self.table_widget._view.item(i, j).setBackground(QColor(int(point1_color[0] * 255), int(point1_color[1] * 255), int(point1_color[2] * 255)))

                    # Set background color for point2 cells (columns 3, 4, and 5)
                    for j in point2_cols:
                        self.table_widget._view.item(i, j).setBackground(QColor(int(point2_color[0] * 255), int(point2_color[1] * 255), int(point2_color[2] * 255)))

                    i += 1

                # also set colormap to the points  
                colors = [colormap(i) for i in range(len(self.geodesic_distmap_marker_layer.data))]
    ...
    def _calculate_local_thickness(self) -> None:
        ...
        self.viewer.add_image(lt.local_thickness(self.labels.data), colormap = 'magma')