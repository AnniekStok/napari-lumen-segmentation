            ...
            if group == "label":
                ...
                if self.label_colormap is not None:
                    ...
                    self.ax.scatter(
                        ...
                        self.props[y_axis_property],
                        c=self.props[group],
                        cmap=self.categorical_cmap,
                        s=10,
                    )
            else:
                # Plot data points on a custom categorical or continuous colormap.
                if self.props[group].dtype == "object" or np.issubdtype(
                    self.props[group].dtype, np.integer
                ):
                    ...
                    if len(unique_categories) <= len(cmap_colors):
                        category_to_color = {
                            category: cmap_colors[i]
                            for i, category in enumerate(unique_categories)
                        }
                        colors = [
                            category_to_color[category]
                            for category in self.props[group]
                        ]
                        self.ax.scatter(
                            self.props[x_axis_property],
                            self.props[y_axis_property],
