...
from qtpy.QtWidgets                         import QVBoxLayout, QWidget, QComboBox, QGroupBox, QPushButton, QScrollArea
...
class SkeletonWidget(QScrollArea):
    ...
    def __init__(self, viewer:napari.Viewer, labels: Labels):
        ...
        self.skeleton_visualization_dropdown.currentIndexChanged.connect(self._update_skeleton_visualization)

        self.skeleton_box_layout.addWidget(self.skeleton_visualization_dropdown)
    ...
    def _skeletonize(self) -> None: 
        ...
        self.random_cmap = mcolors.ListedColormap(extended_color_cycle[:n_colors])

        # define shapes layer        
        self.skeleton = self.viewer.add_shapes(all_paths,
            shape_type='path',
    ...
    def _update_skeleton_visualization(self) -> None:
        ...
        if self.skeleton_visualization_dropdown.currentText() == "Skeleton":
            ...
            self.skeleton.face_color = colors           
            self.table_widget._recolor(by="skeleton-id", cmap = self.random_cmap)    
        if self.skeleton_visualization_dropdown.currentText() == "Branch Length":
